...
OpenLayers.Renderer.PIXI = OpenLayers.Class(OpenLayers.Renderer, {
    ...
    /**
     * Property: canvas
     * {Object} The PIXI container object.
     */
    canvas: null,
    ...
    drawExternalGraphic: function(geometry, style, featureId) {
        ...
        if(!isNaN(p0) && !isNaN(p1)) {
            var filePath = style.externalGraphic;
            var texture;
            if(this.cachedTextures[filePath]) {
                texture = this.cachedTextures[filePath];
            }
            else {
                texture = PIXI.Texture.fromImage(filePath);
                this.cachedTextures[filePath] = texture;
            }
            sprite = new PIXI.Sprite(this.cachedTextures[filePath]);
            sprite.position.x = p0;
            sprite.position.y = p1;
            if(style.rotation) {
                sprite.rotation = (Math.PI / 180.0)*style.rotation;
            }
            this.canvas.addChild(sprite);
            this.features[featureId] = [sprite,style,geometry];
            if(!this.locked) {
    ...
    drawNamedSymbol: function(geometry, style, featureId) {
        ...
        var symbolYCoords = [];
            ...
            for(i = 0; i < symbol.length; i+=2) {
                symbolXCoords.push(symbol[i]);
                symbolYCoords.push(symbol[i+1]);
                symbolBounds.push([symbol[i], symbol[i+1]]);
            }
        var symbolMinX = Math.min.apply(this,symbolXCoords);
        var symbolMaxX = Math.max.apply(this,symbolXCoords);
        var symbolMinY = Math.min.apply(this,symbolYCoords);
        var symbolMaxY = Math.max.apply(this,symbolYCoords);

        var cx = p0 - style.pointRadius;
        var cy = p1 - style.pointRadius;

        var polyBounds = symbolBounds.map(function(point){
            return new PIXI.Point(
                cx+transformX(point[0],symbolMinX,symbolMaxX,style.pointRadius*2),
                cy+transformY(point[1],symbolMinY,symbolMaxY,style.pointRadius*2)
            )
        })
        ...
        graphic.drawPolygon(polyBounds);
