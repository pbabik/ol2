...
OpenLayers.Geometry.LineString = OpenLayers.Class(OpenLayers.Geometry.Curve, {
    ...
    displace: function(x,y,angle,offset) {
        var dx = offset * Math.cos(angle + Math.PI/2)
        dy = offset * Math.sin(angle + Math.PI/2)
        return [x+dx,y+ dy]
    },

    displaceBy: function(x1,y1,x2,y2,x3,y3,offset) {
        var angle1 = this.getSegmentAngle(x1,y1,x2,y2);
        var angle2 = this.getSegmentAngle(x2,y2,x3,y3);
        var sin_a = offset * Math.sin(angle1 + Math.PI/2);
        var cos_a = offset * Math.cos(angle1 + Math.PI/2);
        var h = Math.tan((angle2 - angle2)/2.0);
        var cx = x2 + cos_a - h * sin_a;
        var cy = y2 + sin_a + h * cos_a;
        return [cx,cy];
    },

    getSegmentAngle: function(x1,y1,x2,y2) {
        var dy = y2 - y1;
        var dx = x2 - x1;
    ...
    offset: function(offset) {
        ...
        for(var s=0;s<compLength-1;s++) {
            var angle = this.getSegmentAngle(comps[s].x,comps[s].y,comps[s+1].x,comps[s+1].y);
            if(s===0) {
                offsetCoords.push(this.displace(comps[s].x,comps[s].y,angle,offset));
            }
            else {
                offsetCoords.push(this.displaceBy(
                    comps[s-1].x,comps[s-1].y,
                    comps[s].x,comps[s].y,
                    comps[s+1].x,comps[s+1].y,
                    offset
                  )
        ...
        var finalAngle = this.getSegmentAngle(comps[compLength-2].x,comps[compLength-2].y,comps[compLength-1].x,comps[compLength-1].y);
        offsetCoords.push(this.displace(comps[compLength-1].x,comps[compLength-1].y,finalAngle,offset));
        return new OpenLayers.Geometry.LineString(offsetCoords.map(function(pair){
            return new OpenLayers.Geometry.Point(pair[0],pair[1]);
        }));
    },
    /**
